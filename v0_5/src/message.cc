 ...
 *
 *   <4>  10.02.02   ALX    .- new mpr2(cad,quiet) function. If quiet==1, only copies cad into str_pass, else, like mpr()
 *                          .- bCanUseLastLine==1 means that last ine of screen contains special status of the player
 *
...
//alex:01.02.02       
//The last line can be used to show special status like poisoned, praying, etc                              
int bCanUseLastLine=0;
...
void mpr(const char *inf, int channel = MSGCH_PLAIN, int param = 0,int quiet=false)
{
    char info2[150]; //was [80] {alex:16.02.02} 
    ...
    //If quiet, copy the message in str_pass
    //and doesn't do nothing more
    //Alex:01.03.02
    if (quiet)
       {
          strcpy(str_pass,inf);
    ...
    if (Message_Line == get_number_of_lines() - 18-bCanUseLastLine) // ( Message_Line == 8 )
        more();
    ...
    if (channel != MSGCH_EQUIPMENT)
    {
        ...
        Store_Message[ Next_Message ].text = info2;
...
#ifdef PLAIN_TERM
    gotoxy(2, get_number_of_lines()-bCanUseLastLine);
#endif

#ifdef DOS_TERM
    window(1, 18, 80, 25);
    gotoxy(2, 7-bCanUseLastLine);
#endif
...
//        first_message = 0;
    ...
    for(;;)
    {
        ...
        for (int i = 0; i < num_lines - 2; i++)
        {
            ...
            // avoid wrap-around
            if ((line == first_message && i != 0) || line >=NUM_STORED_MESSAGES)
                break;

            //gotoxy(1,1);
            //cprintf("%d %d %d %d",line,first_message,Next_Message,num_lines);getch();
...
//If quiet, don't does nothing. Only copies
//"inf" into "acr.cc->str_pass".
//
//If not quiet, like mpr()
//Alex:01.03.02
void mpr2(const char *inf,bool quiet=false)
  {
    mpr(inf,MSGCH_PLAIN,0,quiet);
  }  