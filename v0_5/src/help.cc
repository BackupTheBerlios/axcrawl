 ...
 */
int path_build(char *buf, int max, cptr path, cptr file)
{
	/* Special file */
	if (file[0] == '~')
	{
	...
	{
		/* Build the new path */
		snprintf(buf, max, "%s%s%s", path, "/", file);
	}
...
link_blk *next_link(char *links, link_blk *ptr)
{
	...
	while (checking->type != LINK_TYPE_END)
	{
		...
		checking = (link_blk *)checking->next;
	} /* end while */
						...
						if ((*redraw)[1] == 0)
						{
							...
							if (cur_link != NULL)
							{
								...
								while ((*cur_link)->type != LINK_TYPE_END)
								{
									...
									*cur_link = (link_blk *)(*cur_link)->next;
...
bool parse_table(BULP *bptr, char **display, char **links, int *link_no, int *block_no, char *table_tag, char *err_message)
{
	 ...
	 */
	table_info = (int *)calloc(no_cols + 1, sizeof(int));
	...
	else
	{
		...
		C_KILL(table_info, (no_cols+1), int);
...
bool preparse_body(BULP *bptr, char **links_block, int *no_blocks, char *err_message)
{
	...
	*links_block = (char *) malloc(links_size);
...
bool parse_helpfile(BULP *help_file, char **display_blk, char **links_blk, int *no_blocks, char *err_message)
{
	...
	*display_blk = (char *)malloc(display_size);
...
char *help_path_build(char *buffer, char *file, char *lastfile, history_blk *stack)
{
	...
	/* Fill the buffer with the path to the "doc" directory */
	strncpy(buffer,"",1024);

	/* This will give the path without the "doc" bit */
	file_ptr = buffer + strlen(buffer);

	/* We've been passed a full path relative to "doc" */
...
void free_history_stack(history_blk *history)
{
	...
	for (n = 0; n < HISTORY_DEPTH; n++)
	{
		...
		if (history[n].file != NULL)
		{
			...
			rnfree((vptr) history[n].file, XMLHELP_FILENAME_LENGTH + 1);
		}
	}
}
...
void free_file_memory(helpfile_blk *help_file)
{
	...
	{
		rnfree(help_file->display_block, ((int *)help_file->display_block)[0]);
	...
	{
		rnfree(help_file->links_block, ((int *) help_file->links_block)[0]); 
	...
	{
		C_KILL(help_file->block_data, (help_file->no_blks+1), block_blk); 
...
bool init_history_stack(history_blk *history)
{
	...
	for (n = 0; n < HISTORY_DEPTH; n++)
	{
		...
		history[n].file = (char *)ralloc(XMLHELP_FILENAME_LENGTH + 1);
...
bool load_helpfile(char *filename, helpfile_blk *help_file, int width, history_blk *history, char *err_message)
{
	...
	if (bptr == NULL)
	{
		...
		gotoxy(1,1);cprintf(filename);getch();
		path_build(filename, 1024, "doc", "err/notfound.xml");
					   ...
					   err_message) == FALSE)
	{
		...
		path_build(filename, 1024, "doc", "err/notparsed.xml");
...
*err_message, u32b *passback, bool persist, bool dht, char *entry_file)
{
	...
	if (strcmp(filename, "") != 0)
	{
						  ...
						  history, err_message) == FALSE)
		{
							  ...
							  history, err_message) == FALSE)
			{
				...
				Term_putstr(x, y+1, -1, TERM_WHITE,
				err_message);
	...
	while (keypress != CHAR_QUIT)
	{
...
#endif
		if (clear) ClearScreen(1,y + mhd_offset,79,24,0);
		...
		if (dht)
		{
			...
			if (!clear) ClearScreen(1,y + height - 1,79,24);
...
u32b render_xml_file(char *file, bool clear, bool help_text, bool persist)
{
	...
	if ( file != NULL )
	{
		...
		if (strcmp(filename,"") == 0)
		{
			...
			help_path_build(filename,"doc/index.xml", "", history);
		}
	}
	else
	{        
...
/* Cater for unusual behaviour of some Mac compilers */
#ifdef MACINTOSH 
static int i = 0;
#endif /* MACINTOSH */
