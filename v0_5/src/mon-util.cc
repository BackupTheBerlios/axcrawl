...
#include "stuff.h"
#include "view.h"
#include "monmem.h" //alex:13.07.01

//jmf: moved from inside function
 FixedVector < int, NUM_MONSTERS > mon_entry;
...
static int mons_exp_mod(int mclass);
 monsterentry *seekmonster(int *p_monsterid);
...
void mons_init(FixedVector < unsigned short, 1000 > &colour)
{
    ...
    //init monster memories  {alex:11.07.01}
    mon_recall_init();
...
char mons_class_see_invis(int mc)
{
    if (mc == MONS_PLAYER_GHOST || mc == MONS_PANDEMONIUM_DEMON)
        return (ghost.values[3]);
    else if (((seekmonster(&mc))->bitfields & M_SEE_INVIS) != 0)
        return (1);
...
char mons_see_invis( struct monsters *mon )
{
    ...
    c=mons_class_see_invis(mon->type);    
    if (!c) 
      if (scan_mon_inv_randarts( mon, RAP_EYESIGHT ) > 0) c=1;


    return (c); 
}                               // end mons_see_invis()

int mons_class_resist_mag(int mc)
{
    int u = smc->resist_magic;
    char mhd =mons_type_hit_dice(mc);
...
int mons_class_res_negative_energy(int mc)
  {
    if (mons_holiness( mc) == MH_UNDEAD 
        || mons_holiness(mc) == MH_DEMONIC
        || mons_holiness( mc ) == MH_NONLIVING
        || mons_holiness( mc ) == MH_PLANT
        || mc == MONS_SHADOW_DRAGON)
    {
...
int mons_class_res_elec(int mc)
{
    if (mc == MONS_PLAYER_GHOST || mc == MONS_PANDEMONIUM_DEMON)
...
int mons_class_res_poison(int mc)
{
    int u = 0, f = smc->bitfields;
...
int mons_class_res_fire(int mc)
{
    if (mc == MONS_PLAYER_GHOST || mc == MONS_PANDEMONIUM_DEMON)
    {
...
int mons_class_res_cold(int mc)
{
    if (mc == MONS_PLAYER_GHOST || mc == MONS_PANDEMONIUM_DEMON)
    {
...
{
    int u = mons_class_resist_mag(mon->type);
...
int mons_res_poison( struct monsters *mon )
{
    ...
    int u=mons_class_res_poison(mc);
...
int mons_res_fire( struct monsters *mon )
{
    ...
    int u=mons_class_res_fire(mon->type);
...
int mons_res_negative_energy( struct monsters *mon )
{
    ...
    int u = mons_class_res_negative_energy(mc);;
...
// see mons_init for initialization of mon_entry array.
 struct monsterentry *seekmonster(int *p_monsterid)
{
...
//Alex:12.11.01
monsterentry *monster(int n_monster)
  {
    return &mondata[n_monster]; 	
  }	 
