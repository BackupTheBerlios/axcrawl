...
#define _bulp_free(ptr,size) ((void *) rnfree(ptr,size))
#define _bulp_fopen(filename,mode) fopen(filename,mode)
#define _bulp_fclose(fptr) fclose(fptr)
...
int Term_putstr(int x, int y, int n, byte a, cptr s)
{
	...
	//textattr(a);
	//available characters to be printed
	len=strlen(s);
	len_allowed=80-x-len;
	snprintf(buf,len_allowed,s);
	cprintf(buf);textcolor(YELLOW);
	gotoxy(1,1);textcolor(WHITE);cprintf("%s %s",s,buf);getch();
	if (len_allowed<len) 
	  return len_allowed;
	else
	  return 0;  
...
BULP *bulp_open(char *filename, char *mode)
{
	...
	if ( bulp_stricmp(mode, "m") == 0 )
	{
		...
		return_value =(BULP *) malloc(sizeof(BULP));
		...
		/* Here we must load the file into a buffer */
		{
			...
			_bulp_mem *mem_blk =(_bulp_mem *) return_value->_ptr;
			...
			mem_blk->base = (char *)_bulp_malloc(file_size);
...
bool bulp_close(BULP *bulp_ptr)
{
	...
	{
		/* Close the file */
		if ( _bulp_fclose((FILE *)bulp_ptr->_ptr) != 0 )
			return(FALSE);
	...
	if ( bulp_ptr->_bulp_type == _BULP_MEM )
	{
		...
		_bulp_mem *mem_blk = (_bulp_mem *)bulp_ptr->_ptr;
...
int bulp_setpos(BULP *bulp_ptr, u32b *pos)
{
	...
	if ( bulp_ptr->_bulp_type == _BULP_FILE )
	{
		...
		return(fseek((FILE *)bulp_ptr->_ptr, *pos, SEEK_SET));
	}
...
int bulp_seek(BULP *bulp_ptr, s32b pos, int from)
{
	...
	if ( bulp_ptr->_bulp_type == _BULP_FILE )
	{
		...
		return(fseek((FILE *)bulp_ptr->_ptr, pos, from));
	}
...
size_t bulp_read(void *ptr, size_t size, size_t nobj, BULP *bulp_ptr)
{
	...
	if ( bulp_ptr->_bulp_type == _BULP_FILE )
	{
		...
		return(_bulp_fread(ptr, size, nobj, (FILE *)bulp_ptr->_ptr));
	}
