 ...
 *
 *
 *      <5>      16.10.01     ALX      #define USE_NEW_COMBAT_STATS is now an option in options.h
 * 
 *      <5>      14.11.01     ALX     .- print_description  made more useful 
 *                                       and moved to strutil.cc         
 *                                    .- describe_item begins in column 35, not in 25 for readibility
 *                                       and has a frame around the description
 *                                    .- describe_monster calls MonstersRecall()
 *                                    .- append_value() now is public  
 *				      .- describe_ammno uses enum constants instead real values	
 *				      .- new powers for ammno
 *				      .- SPARM_REFLECTION
 *
...
#include "strutil.h" //alex:16.10.01
#include "strutil.h" //alex:14.11.01
#include "monmem.h" //alex:14.11.01
#include "menus.h" //alex:19.11.01
//#include "objects.h" //alex:19.11.01
#include "options.h" //alex:11.12.01
...
//
//---------------------------------------------------------------
void append_value( std::string & description, int valu, bool plussed )
{
    if (valu >= 0 && plussed == 1)
        description += "+";
...
string describe_weapon( const item_def &item, char verbose)
{
    ...
    else
    {
        if (verbose == 1)
        {
            description+=description_weapon(item);
        }
    }
    ...
    if (!is_fixed_artefact( item ))
    {
        ...
        if (spec_ench != SPWPN_NORMAL && item_ident( item, ISFLAG_KNOW_TYPE ))
        {
            ...
            case SPWPN_REACHING:
                ...
                break;
            //AX-Crawl {alex:25.05.05}
            case SPWPN_GOBLIN_SLAYING:
                description += "It is especially effective against goblins and hobglobins.";
                break;     
            }
        }
...
string describe_ammo( const item_def &item )
{
    ...
    case MI_EGGPLANT:
        ...
        break;
    case MI_MAGIC_BALL:
        description+="A little ball with magic energies inside it that are realeased when throw it.";
        break;
    default:
    ...
    if (item.special != 0 && item_ident( item, ISFLAG_KNOW_TYPE ))
    {
        ...
        case SPMSL_POISONED_II:
            ...
            break;
         case SPMSL_SLOW:
            description+="$The victim need more time to move.";
            break;
        case SPMSL_CONFUSE:
            description+="$The victim can find difficult to attack you.";
            break;
        case SPMSL_PARALYSE:
            description+="$It causes paralisys.";
            break;    
        case SPMSL_FIREBALL:
            description+="$It causes at great explosion of fire.";
            break;    
        case SPMSL_ICEBALL:
            description+="$It causes at great explosion of ice.";
            break; 
        case SPMSL_BACKLITE:
            description+="$It backlites a creature and makes it appreciably easier to hit.";
            break;  
        }
    }
...
static std::string describe_armour( const item_def &item, char verbose )
{
    ...
    else
    {
        if (verbose == 1)
        {
            ...
            description += description_armour(item); 
            description += "$";
        }
    }
        ...
        && verbose == 1)
    {
        ...
        case SPARM_REFLECTION:
           description+="It protects you from beams of magic energy, and sometimes, from missiles.";
           break;    
        }
...
string describe_potion( const item_def &item )
{
    ...
    else
    {
        ...
        case POT_MUTATION:
            ...
            break;
        //AX-Crawl {alex:26.02.02}
        case POT_ESP:
            description+="allow you sense the mind of nearby creatures.";
            break;
        case POT_RESTORE_HP_MANA:
            description+="fully revify you and restores all your magic.";
            break;       
        case POT_INMATERIALITY:
            description+="eables you to ignore the matery, making you invulnarable to phisic damage,"
                         "but also uneables you to take objects from floor and to damage any creature "
                         "with phisics ways.";
            break;
        case POT_AGGRAVATE:
            description+="causes your body emmit a strange radiation that alerts of your presence all nearby creatures.";
            break;
         case POT_HUNGER:
            description+="makes you more hunger.";
            break;
        }
...
string describe_scroll( const item_def &item )
{
    ...
    else
    {
        ...
        case SCR_ENCHANT_WEAPON_III:
            ...
            break;
        case SCR_INVULNERABILITY:
            description+="After reading this scroll, nothing could harm you.";
            break;
        case SCR_DETECT_DANGER:
            description+="This scrolls helps you avoid dangerous creatures and traps.";
            break;
        case SCR_MASS_CONFUSION:
            description+="After reading this scroll, nearby monsters will find more difficult harm you.";
            break;
        default:
    ...
    else
    {
        if (verbose == 1 || is_random_artefact( item ))
        {
            ...
            case AMU_RESIST_MUTATION:
                ...
                break;
            //alex:21.12.01
            case AMU_FORTUNE_GIFT:
                description += "The unpredictible goddess Fortune kissed the amulet, "
                               "and offered to the mortals as a gift ... and as a joke. "
                               "Only the stupid or the really very valient would rely on it to survive. "
                               "It will help you but, like Fortune, it will does capriciously and "
                               "it will fail you in the worst moment.";
                 break;
            case AMU_BEAUTIFUL:
               description+="This is a beatiful amulet, uselessness to nothing, only to decorate your neck and improve your appearance.";
               break;
            case AMU_DEATH_EVADER:
                 description +="This amulet pulses plenty of life. Long time ago, a wizard discovered "
                               "the essence of life and put it inside this amulet. When the wearer of "
                               "the amulet dies, sometimes The Death is confused and takes the essence "
                               "of the amulet instead the life of the wearer. Remember that Death is very very old "
                               "and sometimes won't be confused."; 
                 break;              
            case AMU_FATALITY:                  
                 description+="Only very stupid people would wear this amulet. It raises the experience"
                              "for each kill, but also increases the probability of being death."
                              "It gives you a malediction that calls a nasty bad luck over your."
                              "The amulet auto-cursed and when removed, the malediction remains 1000 turns at minumun";  
                 break;             
            case AMU_LIFE_EATER:
                 description+="If you don't want die, don't wear this amulet.$ Each turn has a chance of eat a bit of your life.$Also, each turn, it can cursed itself";
                 break;
            case AMU_LIFE_SUPLY:
                 description+="This amulet stores life. When necessary, the user drains some life from the amulet."
                              "The amulet, sometimes, uses gained experience to recharge itself. "
                              "$Also, it partially protects you from negative energy. ";
                 append_value(description,you.inv[you.equip[EQ_AMULET]].plus,false);             
                 break;             
            case RING_AGGRAVATE:
                 description+="The ring alerts the creatures of the dungeon of your presence.";
                 break;
            case RING_CONFLICT:
                 description+="All the creatures will see an enemy in the others.";
                 break;          
            default:
