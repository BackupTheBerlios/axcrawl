 ...
 *
 *   <8>   25.03.02      ALX    .- xtraOps.no_stair_delay
 *                              .- user defined screens
 *                              .- New function name_save_fich(). Handle extra info of AX-Crawl
 *                              .- make_filename returns "save/nom_player/file.ext"
 *                              .- Now savegames are stored in subdirectory save/nom_player/
 *
...
#include "options.h" //alex:24.03.02
#include "userscr.h" //alex:21.03.02
#include "usestair.h" //alex:24.03.02
#include "monmem.h" //alex:24.03.02

void save_xtra();     //alex:18.07.01
void restore_xtra();  //alex:18.07.01
...
//Returns in charFile "save/you.your_name/"
//Alex: 12.11.01
void CreateDirSaveFile(char *charFile)
  { 
    int n; 	    
    strcpy(charFile,"save/");
    
    n=mkdir(charFile,0700);
    strcat(charFile, you.your_name);
    strcat(charFile, "/");  
    n=mkdir(charFile,0700);   
  }
...
//returns a string:
//
//  save/your_name/your_name.extension
void name_save_fich(char *charFile, char *extension)
  {    
    char cmd_buff[1024];
    ...
    sprintf(charFile, "%s.%s", name_buff,extension);
#else
    CreateDirSaveFile(charFile);
    //strncpy(charFile, you.your_name, kFileNameLen);
    //charFile[kFileNameLen] = 0;
    strcat(charFile,you.your_name);
    strcat(charFile, extension);
    ...
    bool isLabyrinth, bool isGhost)
{
    ...
    //bones are in crawl/bones/ directory
    if (strcmp(prefix,"bones")!=0 && !isGhost)
       CreateDirSaveFile(finalprefix);
    else
       strcpy(finalprefix,"bones");   
    strcat(finalprefix, prefix);
    strcpy(buf, finalprefix);
    ...
    {                           
      ...
      out_of_foll:
        ...
        if (load_mode == LOAD_ENTER_LEVEL)
        {
            ...
            //Avoid stair delay {alex:24.03.02    }
            if (!xtraOps.ax_no_stair_delay)    
              {
...
void save_game(bool leave_game)
{
    ...
    if (!see_you_soon()) cprintf( "See you soon, %s!", you.your_name );    
...
//Addes bOnlyVerify. When true, only verifies save game and return cero
//if some problem exists with savefile
//alex:19.09.01
int restore_game(int bOnlyVerify=0)
{
    char char_f[kFileNameSize];

    name_save_fich(char_f,".sav");
    ...
    if (!feof(restoreFile))
    {
        if (bOnlyVerify) {fclose(restoreFile);return 0;}
        snprintf( info, INFO_SIZE, "\nIncomplete read of \"%s\" - aborting.\n", char_f);
        perror(info);
        end(-1);
    }
    ...
    fclose(restoreFile);
    //read extra information for my patch {alex:18.07.01}
    if (!bOnlyVerify) restore_xtra();
    return 1;
}
...
/* save_xtra()
   ...
   Saves variables extra from my patch:
   
   .- monster memories
   .- var_log (used by flog() to show only
      the dungeon level when the level changes  
...
void save_xtra()
  {
    ...
    //open file NamePlayer.xtra
    name_save_fich(xtrafile,".xtra");
    FILE *saveFile = fopen(xtrafile, "wb");

    if (saveFile == NULL)
    {
        strcpy(info, "Unable to open \"");
        strcat(info, xtrafile );
        strcat(info, "\" for writing!");
        perror(info);
        end(-1);
    }
   
    //0.- Versión of savefile
    fwrite(&version,sizeof(version),1,saveFile); //actual version: 1
    
    //1.- monster memories
    fwrite(monsters_recall,sizeof(tp_monsters_recall),NUM_MONSTERS,saveFile);
    
    //2.- var_log
    fwrite(&var_log,sizeof(tp_var_log),1,saveFile);
    
    //3.- turns
    fwrite(&turns,sizeof(turns),1,saveFile);     
    
    //4.- options
    fwrite(&xtraOps,sizeof(xtraOps),1,saveFile);
    
    //close file
    fclose(saveFile);     
    
    //clrscr();
    //cprintf("%d",xtraOps.ax_autopickups,xtraOps.ax_no_pick_bad_potions);getch();
  }             


/* restore_xtra()
  
   restore variables extra from my patch:
...
void restore_xtra()
{
    ...
    //open file NamePlayer.xtra
    name_save_fich(xtrafile,".xtra");
    FILE *saveFile = fopen(xtrafile, "rb+");
    ...
    //0.- Version of savefile
    fread(&version,sizeof(version),1,saveFile);
    
    //1.- monster memories
    fread(monsters_recall,sizeof(tp_monsters_recall),NUM_MONSTERS,saveFile);
    
    //2.- var_log
    fread(&var_log,sizeof(tp_var_log),1,saveFile);
    
    //3.- turns
    fread(&turns,sizeof(turns),1,saveFile);    
    
    //4.- turns
    fread(&xtraOps,sizeof(xtraOps),1,saveFile);    
    
    //close file
    fclose(saveFile); 
    
    //saved autopickup options take precedence over init.txt
    Options.autopickups=xtraOps.ax_autopickups;
  }