 ...
 *
 *
 *      <9>      21.03.02       ALX     .- new score list system (info about weapon/missile that killed the player)
 *                                      .- resting   
 *
...
void ouch(int dam, int death_source, char death_type)
{
    ...
    //{alex:15.11.01}
    if (dam<=0 && death_type!=KILLED_BY_QUITTING) return;
    //When you take damage, stop resting
    stop_resting(); 
    //---
...
//----------------------      
//Option to avoid death
//Alex:25.10.01
    if (xtraOps.ax_cheat_dead)
      {      	
          if (death_type != KILLED_BY_QUITTING && death_type != KILLED_BY_WINNING && death_type != KILLED_BY_LEAVING)
           {
              if (!yesno("Allow Die?", false))
                 {
                    set_hp(you.hp_max, false);
                    xtraOps.nDeathAvoids++;
                    return;
                  }
           }
      }            
//----------------------      
    ...
    se.death_weapon[0]  = '\0';  //alex:11.03.02
    se.death_missile[0] = '\0';  //alex:20.03.02
    ...
    if (monster != NULL)
    {
        ...
        if (monster->inv[MSLOT_WEAPON] != NON_ITEM)
        {
            set_ident_flags( mitm[monster->inv[MSLOT_WEAPON]], 
                             ISFLAG_IDENT_MASK );
            //name of the weapon {alex:11.03.02}
            it_name(  monster->inv[MSLOT_WEAPON], DESC_NOCAP_A, se.death_weapon);                    
        }

        if (monster->inv[MSLOT_MISSILE] != NON_ITEM)
        {
            set_ident_flags( mitm[monster->inv[MSLOT_MISSILE]], ISFLAG_IDENT_MASK );
            //name of the missile {alex:20.03.02}
            //We don't want the number of missiles that the monster has
            int quantity;
            quantity=mitm[monster->inv[MSLOT_MISSILE]].quantity;
            mitm[monster->inv[MSLOT_MISSILE]].quantity=1;
            it_name(  monster->inv[MSLOT_MISSILE], DESC_NOCAP_A, se.death_missile);                    
            mitm[monster->inv[MSLOT_MISSILE]].quantity=quantity;
        }
