 ...
 *
 * <10> 21.03.02  ALX  .- Ask for an item of the inventory and destroy it 
 *                     .- bug in autopickup
 *                     .- no autopickup rotting corpses 
 *                     .- function to print the name of an object class
 *
...
#include "view.h" //alex:21.03.02
#include "strutil.h" //alex:21.03.02
...
init_pickup: //alex:10.09.01}
    ...
    while (o != NON_ITEM)
    {
        ...
        {
            //--- 	
            //autopickup must skip rotting corpses {alex:10.09.01}
            if (!(mitm[o].base_type == OBJ_FOOD && mitm[o].sub_type == FOOD_CHUNK && mitm[o].special<100))            
             {
            //--- 	
                 ...
                 did_pickup = true;
                 //after pick something, mitm[o].link==NON_ITEM
                 //but sometimes they are more items or another class to pickup
                 //Alex: 16.11.01
                 goto init_pickup;
                 //---
             } //autopickup must skip rotting corpses {alex:10.19.01}      
...
/*-----------------------------------
  ...
  Ask for an item of the inventory and destroy it
...
{ 
    ...
    item=prompt_invent_item("Destroy which item? ", -1,1);
    if (item == PROMPT_ABORT) return;
    
    //don't destroy cursed items
    if (item_cursed(you.inv[item]))
      {
      	mesclr();
      	mpr("You can't destroy a cursed item",MSGCH_WARN);
...
//prints the name of an object class
//alex:05.02.02
void PrintNameObjectClass(int base_type)
  {
       switch (base_type)
         {
                ...
                case OBJ_GEMSTONES:  cprintf("Gem Stones"); break; //removed comment {alex:05.02.02}
                default: cprintf("Bug");
         }
   }         
...
{ 
    ...
    //ask the item to destroy    
    mesclr();
    item=prompt_invent_item("Inscribe which item? ", -1,1);
    if (item == PROMPT_ABORT) return;
    
    //ask the inscription    
    in_name( item, DESC_INVENTORY_EQUIP, str_pass );
    textcolor(LIGHTBLUE);
    gotoxy(1,nrow);cprintf("Enter inscription for ");
    textcolor(WHITE);
    cprintf(str_pass);
    textcolor(LIGHTBLUE);
    cprintf(" (ESC=abort)",str_pass);    
    textcolor(LIGHTGREY);
    askfor_aux(you.inv[item].inscription,30,1,nrow+2,0);
    gotoxy(1,nrow);
    cprintf("                                                                             ");
    gotoxy(1,nrow+1);
    cprintf("                                                                             ");
