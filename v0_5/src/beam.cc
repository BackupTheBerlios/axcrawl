 ...
 *
 *   <8>    21.03.02     ALX    beams knows the exact cause of death
 *
...
bool check_mons_resist_dispel_undead( struct monsters *monster, int pow )
{
    int res = mons_resist_turn_undead( monster );
    ...
    const int mrchance = (100 + res) - pow;
    const int mrch2 = random2(100) + random2(101);
...
static int affect_player(struct bolt &beam)
{
    ...
    else
    {
        ...
        case BEAM_PAIN:      // pain
            ...
            if (YOU_KILL(beam.thrower))
                //ouch(roll_dice(beam.damage), 0, KILLED_BY_TARGETTING); //alex:21.03.02
                ouch(roll_dice(beam.damage), 0, KILLED_BY_TARGETTING_PAIN);                
            else
                //ouch(roll_dice(beam.damage), beam.beam_source, KILLED_BY_BEAM); //alex:21.03.02
                ouch(1 + random2(beam.hit), beam.beam_source,KILLED_BY_PAIN);
        ...
        case BEAM_DISPEL_UNDEAD:
            ...
            if (YOU_KILL(beam.thrower))
                //ouch(roll_dice(beam.damage), 0, KILLED_BY_TARGETTING); {alex:21.03.02}
                ouch(random2(beam.hit), 0, KILLED_BY_TARGETTING_DISPEL_UNDEAD);
            else
                //ouch(roll_dice(beam.damage), beam.beam_source, KILLED_BY_BEAM); //alex:21.03.02
                ouch(random2(beam.hit), 0, KILLED_BY_DISPEL_UNDEAD);
        ...
        case BEAM_DISINTEGRATION:
            ...
            if (YOU_KILL(beam.thrower))
                //ouch(roll_dice(beam.damage), 0, KILLED_BY_TARGETTING); {alex:21.03.02}
                ouch(roll_dice(beam.damage), 0, KILLED_BY_TARGETTING_DISINTEGRATION); 
            else
                //ouch(roll_dice(beam.damage), beam.beam_source, KILLED_BY_BEAM); {alex:21.03.02}
                ouch(roll_dice(beam.damage), beam.beam_source, KILLED_BY_BEAM_DISINTEGRATION);
    ...
    else
    {
        char killer = (beam.flavour == BEAM_SPORE)?KILLED_BY_SPORE:
            beam.thrower; //was KILLED_BY_BEAM {alex:20.03.02}
...
static int affect_monster_enchantment(struct bolt &beam, struct monsters *mon)
{
    ...
    if (beam.flavour == BEAM_DISPEL_UNDEAD)
    {
        ...
        if (check_mons_resist_dispel_undead(mon, beam.ench_power))
            return MON_RESIST;
    ...
    if ((beam.flavour == BEAM_ENSLAVE_UNDEAD 
        ...
        || (beam.flavour == BEAM_ENSLAVE_DEMON 
    ...
    //
    if (check_mons_magres(mon, beam.ench_power)
        && beam.flavour != BEAM_HASTE && beam.flavour != BEAM_HEALING
        && beam.flavour != BEAM_INVISIBILITY)
    {
        return MON_RESIST;
    }
