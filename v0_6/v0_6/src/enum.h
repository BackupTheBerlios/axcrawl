...
enum COMMANDS
{
    ...
    CMD_SAVE_GAME_NOT_EXIT,  //save but not exit
    //alex:16.10.01
    CMD_DESTROY_ITEM,  //destroy a item
    CMD_BROWSE_BOOK,   //browse a book
    //alex:29.11.01
    CMD_INSCRIPTION,   //give name to an object
    CMD_DESC_WEAP      //detailed description of a weapon
...
enum ITEM_TYPE_ID_STATE  // used for values in id[4][50]
{
    ...
    //alex:19.11.01    
    ID_NO_PICK,  //know item. It must be skipped from autopickup
    ID_PICK      //know item. It can be auto-picked 
};
...
enum JEWELLERY
{
    ...
    AMU_RAGE=35, //in ax-crawl, the distinction between amulets and rings is in the external file {alex:26.03.02} = 35,                     //   35
    AMU_RESIST_SLOW,
    AMU_CLARITY,
    AMU_WARDING,
...
enum JOB
{
    ...
    JOB_TRANSMUTER,
    JOB_HEALER,                        //   22
    //JOB_QUITTER, {aleX:20.03.02}                       //   23 -- this is job 'x', don't use
    JOB_REAVER,                        //   24
    JOB_STALKER,                       //   25
    JOB_MONK,
    JOB_WARPER,
...
enum KILLBY
{
    ...
    KILLED_BY_ACID,
    //AX-Crawl deaths that are not by a monster
    //alex:26.03.02
    KILLED_BY_SCR_IMMOLATION,
    
    //alex:20.03.02
    //specify the spell/wand that kill the player
    //Up to now, only used KILL_MON_MISSILE
    
    KILLED_BY_MON_MISSILE,
    //wands and spells
    KILLED_BY_HELLFIRE,
    KILLED_BY_SMITING,
    KILLED_BY_MMISSILE,
    KILLED_BY_FLAME,
    KILLED_BY_FROST,
    KILLED_BY_VENOM_BOLT,
    KILLED_BY_FIRE_BOLT,
    KILLED_BY_COLD_BOLT,
    KILLED_BY_LIGHTNING_BOLT,
    KILLED_BY_CRYSTAL_SPEAR,
    KILLED_BY_NEGATIVE_BOLT,
    KILLED_BY_ORB_ENERGY,
    KILLED_BY_STEAM_BALL,
    KILLED_BY_PAIN,
    KILLED_BY_STICKY_FLAME,
    KILLED_BY_POISON_BLAST,
    KILLED_BY_PURPLE_BLAST,
    KILLED_BY_ENERGY_BOLT,
    KILLED_BY_STING,
    KILLED_BY_STONE_ARROW,
    KILLED_BY_POISON_SPLASH,
    KILLED_BY_DISINTEGRATE,
    KILLED_BY_MARSH_GAS,
    KILLED_BY_QUICKSILVER_BOLT,
    KILLED_BY_FIREBALL,
    KILLED_BY_METAL_SPLINTERS,        
    KILLED_BY_TARGETTING_PAIN, //killed by spell of pain when low hp
    KILLED_BY_TARGETTING_DISINTEGRATION, //killed by bad use of spell
    KILLED_BY_BEAM_DISINTEGRATION,   
    KILLED_BY_IRON_BOLT,
    KILLED_BY_TARGETTING_DISPEL_UNDEAD,
    KILLED_BY_DISPEL_UNDEAD,

   
    //special attacks    
    KILLED_BY_GLOB_LAVA, //glob of lava
    KILLED_BY_ELECTRICITY, //electric eels
    KILLED_BY_VOLLEY_OF_SPIKES, //manticore
    KILLED_BY_SPIT_ACID,   //plants
    KILLED_BY_BREATHING_FLAMES, //dragons of fire
    KILLED_BY_BREATHING_COLD, //dragons of ice
    NUM_KILLBY
};
...
enum MISSILES                          // (unsigned char)
{
    ...
    //AX-Crawl missils {alex:18.02.02}
    MI_MAGIC_BALL,  //no phisic, damage, only magic damage
    NUM_MISSILES,
    MI_EGGPLANT
};
...
enum POTIONS
{
    ...
    POT_CURE_MUTATION,
    POT_MUTATION,
    //AX-Crawl good potions {alex:26.02.02}
    POT_ESP,   
    POT_RESTORE_HP_MANA,
    POT_INMATERIALITY, //you can pass throug walls
    //AX-Crawl bad potions,
    POT_HUNGER,
    POT_AGGRAVATE,
    NUM_POTIONS
};
...
enum SPECIAL_WEAPONS // equivalent to (you.inv[].special or mitm[].special) % 30
{
    ...
    SPWPN_ELECTROCUTION,
    //SPWPN_ORC_SLAYING,                 //    5
    SPWPN_DUMMY, //waste the position of orc_slaying
    SPWPN_VENOM,
    SPWPN_PROTECTION,
    SPWPN_DRAINING,
    SPWPN_SPEED,
    ...
    SPWPN_RANDART_IV,
    SPWPN_RANDART_V,
    //AX-Crawl {alex:22.02.02}
    SPWPN_SLAYING,                   // 30                  
    SPWPN_ORC_SLAYING,             
    SPWPN_GOBLIN_SLAYING,
    SPWPN_DEMON_SLAYING,
    SPWPN_UNDEAD_SLAYING,
    SPWPN_LAST_SLAYING,
    NUM_SPECIAL_WEAPONS=SPWPN_LAST_SLAYING,
...
enum EFFECTS
  {
     ...
     EFE_BLINK                ,
     EFE_TORMENT              ,
     EFE_ENCHANT_WEAPON_III   ,
     EFE_SURE_VORPALISE       ,
     EFE_VORPALISE            ,
     EFE_INVULNERABILITY      ,
     EFE_IMMOLATION           ,
     EFE_PAPER,
     EFE_RANDOM_USELESSNESS,
     EFE_ACQUIREMENT,
     EFE_SUMMONING,
     EFE_RECHARGING,
     EFE_DETECT_DANGER,
     ...
     //from potions
     EFE_POTIONS=EFE_LAST_SCROLL,
     EFE_INMATERIALITY=EFE_POTIONS        ,
     EFE_INVISIBILITY         ,
     EFE_PARALISYS            ,
     EFE_BERSERKER            ,
     EFE_RESTORE_HP_MANA      ,
