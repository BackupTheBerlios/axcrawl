...
newgame:        
    ...
    if (you.your_name[0]!=0) return false;
...
int p_item2(struct tMenu& mnu,int bSelec)
  {
     int n;
     struct tpClass *job;
     char *cad;
     ...
     if (bSelec==4) 
     ...
     else   
        {
          if (bSelec)
             ...
             textbackground(BLACK);          
          job=(struct tpClass *)mnu.data+mnu.nItem;         
          cad=job->job;
          if (job->bOk)
             textcolor(WHITE);
          else
...
/* handle_key for choose_race()
   handle_key2 for choose_class()
...
int handle_key2(struct tMenu& mnu,int c) 
 {
   int n;
   struct tpClass *job;
   ...
   for (n=0;n<mnu.nMaxItems;n++)
     {
       job=(struct tpClass *)mnu.data+n;         
       if (job->job[0] == c) return n;	
     }
   return -2;  		
 } 	
...
bool choose_class(void)
{
    ...
    int nItem;
    //char jobs[35][25];
    struct tpClass jobs[NUM_JOBS+1],jb; 
...
job_query:

    {
        mnu.nFilaMax=23;
        mnu.nFilaIni=5;        
        mnu.print_item=p_item2;
        mnu.handle_key=handle_key2;
        mnu.data=(char *)jobs;
        mnu.nMaxItems=-1;
        ...
        {            
            bOk=class_allowed(you.species, i);
            if (!bOk)
              if (!Options.allow_all_combos) continue;        	
         
            mnu.nMaxItems++;	   
            st_prn[0]=index_to_letter(i);
            st_prn[1]='\0';
            
            strcpy(jb.job,st_prn);
            strcat(jb.job," - ");
            strncat(jb.job,job_title(i),16);
            jb.bOk=bOk;   
            jobs[mnu.nMaxItems]=jb; 
        }
        
        mnu.nMaxItems++;strcpy(jb.job,"? - Random");jb.bOk=1;jobs[mnu.nMaxItems]=jb;
        mnu.nMaxItems++;strcpy(jb.job,"X - Back");jobs[mnu.nMaxItems]=jb;
        mnu.nMaxItems++;strcpy(jb.job,"Q - Quit");jobs[mnu.nMaxItems]=jb;
        
        //cprintf(EOL "? - Random; x - Back to species selection; X - Quit" EOL);
        //cprintf(EOL "What kind of character are you? ");
        
        /*for(i=0;i<=mnu.nMaxItems;i++)
          {
            job=(struct tpClass *)mnu.data+i;	
            gotoxy(1,1);cprintf("%s %s",jobs[i].job,job->job);
            cprintf("                                   ");
            getch();	
          }*/	
