 ...
 *
 *   <9>     26.03.02    ALX    standard_name_xxxx() uses aObjs    
 *
...
#include "objects.h" //alex:26.03.02
...
{
    char *name;        // true name of unrandart (max 31 chars)
    char *unid_name;   // un-id'd name of unrandart (max 31 chars)
...
{
    strcpy(glorg,aObjs[OBJ_WEAPONS][item_typ].name);      
...
void standard_name_armour( const item_def &item, char glorg[80] )
{
    ...
    strcpy(glorg, aObjs[OBJ_ARMOUR][item.sub_type].name);
