 ...
 *
 *   <12>     16.02.02   ALX    Changed the way friendly monsters are coloured                        
 *
 *   <11>     19.12.01   ALX    .- show_map can be dumped to a file
 *                              .- draw_border takes extra params when rolling a player  
 *                             
...
#include "stdio.h"  //alex:19.12.01
#include "stdlib.h" //alex:19.12.01
...
//alex:16.02.02
//to higlite pets
int bHighLiteFriends=0;

#include "usestair.h" //alex:21.03.02
...
//Alex 16.02.02: Changed the way friendly monsters are coloured                        
//#ifdef USE_COLOUR_OPTS
            if (mons_friendly(monster) && bHighLiteFriends>0)
            {
            	int color;
            	switch(bHighLiteFriends)
            	  {
            	     case 1:
            	        color=DARKGREY;break;
            	     case 2:
...
// this bit draws the borders:
#ifdef DOS_TERM
    if (bClearScreen) window(1, 1, 80, 25);
#endif

    if (bClearScreen) clrscr();
    gotoxy(40, nRowIni-2);
    textcolor(LIGHTGREY);
    char print_it[80];
    char print_it2[42];
    ...
    gotoxy(40, nRowIni-2);
    textcolor(YELLOW); //before LIGHTGRAY {alex:18.09.01}
    cprintf(print_it2);
    gotoxy(40, nRowIni-1);
...
//
// If toFile is specified, then the map is dumped to the that file
// Alex: 19.12.01
...
void show_map( FixedVector<int, 2> &spec_place,char *toFile=NULL )
{
    ...
    if (toFile!=NULL)
       {
         fich=fopen(toFile,"wt");
         level_name(str_pass);
         fprintf(fich,"%s\n-----------------------------------------------------------------\n\n",str_pass);
         fprintf(fich,"Turns: %d      Exp. Level: %d    Exp. Points: %d \n",turns,you.experience_level,you.experience);
       }  
    else
       fich=NULL;   
  ...
  put_screen:
    ...
    //when dump to a file, show all lines
    //else, only the number of lines that fit on the screen
    //Alex:20.12.01
    if (toFile) num_lines=map_lines;
    
    for (j = 0; j < num_lines; j++)
    {
    	if (j > 0 && fich!=NULL)  {more();fprintf(fich,"\n");}
        for (i = 0; i < 80; i++)
        {
...
#ifdef DOS_TERM
                if (fich!=NULL)
                   {more();fprintf(fich,"%c",buffer2[bufcount2-2] == 0 ? 32 : buffer2[bufcount2-2]);}
                continue;
#endif
            ...
            buffer2[bufcount2 + 1] = colour_code_map(start_x + i + 1,start_y + j + 1);
...
print_it:            
            if (fich!=NULL)
              {              	 
                 more();fprintf(fich,"%c",buffer2[bufcount2]==0 ? 32 : buffer2[bufcount2]);
              }      
...
#ifdef DOS_TERM
    if (fich==NULL)  puttext(1, 1, 80, 25, buffer2);    
#endif
  
gettything:
  if (fich!=NULL) 
     {
       more();fclose(fich);goto putty;
     }	
  else       
      {
         _setcursortype(_NORMALCURSOR);
