 ...
 *
 *      <4>     21.03.02        ALX       .- all combinations of classes/races allowed by ini.txt or command line
 *                                        .- new command line param "-doc" to see documentation
 *                                        .- new command line param "-nointro" to skip intro screen
 *					  .- new command line param "-new" to begin with a new game
 *					  .- new command line param "-last" to begin with last played game
 *                                        .- new entries in init.txt for Ax-Crawl
 *                                       
...
#include "options.h" //alex:25.10.01
...
void read_init_file(void)
{
    ...
    Options.newgame       = 0;
    Options.last          = 0; 
    Options.allow_all_combos=0; //alex:21.03.02
    ...
    while (!feof(f))
    {
      ...
      //AX-Crawl options {alex:21.03.02}
       else if (key == "ax_allow_all_combos")
        {
            // allow of combinations of races/clases
            Options.allow_all_combos = read_bool( field, Options.allow_all_combos );
        }
      else if (key == "ax_nointro")
        {
            // skip intro screen
            Options.skip_intro = read_bool( field, Options.skip_intro );
        }
      else if (key == "ax_no_stair_delay")
        {
            // skip intro screen
            xtraOps.ax_no_stair_delay = read_bool( field, xtraOps.ax_no_stair_delay );
        }                                                   
       //24.09.01
        else if (key=="ax_show_weight")  xtraOps.ax_show_weight=read_bool( field, xtraOps.ax_show_weight);
        else if (key=="ax_dump_with_monster_kills")  xtraOps.ax_dump_with_monster_kills=read_bool( field, xtraOps.ax_dump_with_monster_kills);
        //25.10.01
        else if (key=="ax_cheat_dead")  xtraOps.ax_cheat_dead=read_bool( field, xtraOps.ax_cheat_dead);
        else if (key=="ax_show_dam")  xtraOps.ax_show_dam=read_bool( field, xtraOps.ax_show_dam);  
        //29.11.01
        else if (key=="ax_show_inscriptions")  xtraOps.ax_show_inscriptions=read_bool( field, xtraOps.ax_show_inscriptions);
        else if (key=="ax_merge_inscriptions")  xtraOps.ax_merge_inscriptions=read_bool( field, xtraOps.ax_merge_inscriptions);        
        //05.02.02
        else if (key=="ax_no_pick_empty_wands") xtraOps.ax_no_pick_empty_wands=read_bool( field, xtraOps.ax_no_pick_empty_wands);
        //24.03.02
        else if (key=="ax_no_stair_delay") xtraOps.ax_no_stair_delay=read_bool( field, xtraOps.ax_no_stair_delay);
    }
...
static char *cmd_ops[] = { "scores", "name", "race", "class",
                           "pizza", "plain", "dir", "rc",
                           "wiz",
                           "allcombos","doc","nointro","new","last"}; //added  {alex:21.03.02}

const int num_cmd_ops = 14; //was "...= 8" {alex:21.03.02}
    ...
    while(current < argc)
    {
        ...
        switch(o)
        {
            ...
            case 7:
                ...
                break;
            //AX-Crawl
            case 9: //Allow of combinatins of races/classes {alex:21.03.02}
                Options.allow_all_combos=true; 
                break;    
            case 10: //show docs
                do_help();
                exit(0);
