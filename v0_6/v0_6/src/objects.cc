...
char *aDescObjs[NUM_OBJECT_CLASSES];

//holds an array with the data of the weapons
TWeapon *aObjs[NUM_OBJECT_CLASSES];

//number of objects
int NObjs[NUM_OBJECT_CLASSES];
...
void init_item(int nWich,int nItem)
  {
    ...
    aObjs[nWich][nItem].id=ID_UNKNOWN_TYPE;
    aObjs[nWich][nItem].isgood=0;
    switch(nWich)
      {
      	case OBJ_POTIONS:
...
void load_weapons(char *filename,int nWich)
 {
   ...
   int nNdxUnId; //index in aPtrDescUnId
   int n,m;
   char *p;
   ...
   while (1)
     {
       ...
       if (strncmp(buf,"ID",2) ==0)
          {
               ...
               *p='\0';
               ...
               //next un-identified text for this object
               //but not all objects have un-id texts               
               n=NDescUnId[nWich];
               nNdxUnId++;
               if (n==-1) continue;
               //if there are less un-id. texts than items ...               
               if (nNdxUnId>NDescUnId[nWich]) nNdxUnId=0;
               aObjs[nWich][nWeapon].unid=nNdxUnId;
               /*if (nWich==OBJ_WANDS)
                 {printf("%d %d %s\n\r",nIdItem,nNdxUnId,uniddesc(nWich,nWeapon));
                  getch();}*/
               continue;
           }
        //name of the weapon
       ...
       //confuse effect
       if (strncmp(buf,"CF",2)==0)
          {
               aObjs[nWich][nWeapon].effect_confused=ParseFlag(&buf[3],aEffects,NUM_EFFECTS);
               continue;
          }
       //level
...
void init_objects()
   {  
      load_uniddesc("unidpot.txt",OBJ_POTIONS);
      load_uniddesc("unidwnd.txt",OBJ_WANDS);
      load_uniddesc("unidamu.txt",OBJ_AMULETS);
      load_uniddesc("unidrng.txt",OBJ_RINGS);
      load_uniddesc("unidbks.txt ",OBJ_BOOKS);
      load_uniddesc("unidstv.txt",OBJ_STAVES);
     
      load_weapons("potions.txt",OBJ_POTIONS);      
      load_weapons("weapons.txt",OBJ_WEAPONS);
      load_weapons("missil.txt",OBJ_MISSILES);
      load_weapons("armour.txt",OBJ_ARMOUR);
      load_weapons("scrolls.txt",OBJ_SCROLLS);
      load_weapons("wands.txt",OBJ_WANDS);
      load_weapons("books.txt",OBJ_BOOKS);
      load_weapons("staves.txt",OBJ_STAVES);
      
      load_weapons("rings.txt",OBJ_RINGS);
      load_weapons("amulets.txt",OBJ_AMULETS); 
      ...
      clrscr();
      for(n=0;n<=NObjs[OBJ_STAVES];n++)
        {
          item.sub_type=n;         
          NameItem(item,buf);
          cprintf("%d %s -- ",n,buf);
          aObjs[OBJ_STAVES][n].id=ID_KNOWN_TYPE;    
          NameItem(item,buf);
          cprintf("%s\n\r",buf);
        }  
      getch();  
