 ...
 *
 *  <12>   11.12.01      ALX    .- #define USE_NEW_COMBAT_STATS is now an option in options.h
 *                              .- ax_rules
 *
...
#include "options.h" //alex:18.07.01
#include "monmem.h"  //alex:18.07.01
...
//they where inline. {alex:11.12.01}
static int player_weapon_str_weight( int weapon = -1);
static int player_weapon_dex_weight( int weapon = -1);
...
//#ifdef USE_NEW_COMBAT_STATS  //alex:11.12.01
if (xtraOps.USE_NEW_COMBAT_STATS)
 {
    int str_weight;
    if (wepType == -1)
...
void you_attack(int monster_attacked, bool unarmed_attacks)
{
    ...
    char damage_noise[20], damage_noise2[20];
 ...
 */
    ...
    //{alex:12.11.01}your_to_hit = 15 + (calc_stat_to_hit_base() / 2);
    your_to_hit = calc_stat_to_hit_base();
    ...
    if (ur_armed)
    {
        ...
        /* magical staff */
        if (item_is_staff( you.inv[ weapon ] ))
            your_to_hit += property( you.inv[ weapon ], PWPN_HIT );
    }
...
#if DEBUG_DIAGNOSTICS
    snprintf( info, INFO_SIZE, "Weapon speed: %d; min: %d; attack time: %d", 
              weapon_speed2, min_speed, you.time_taken );
    ...
    */
    ...
    if (xtraOps.ax_fight_rules)
    //If you really hit
    //or you stab (includes defender->behavior == BEH_SLEEP)
    //or monster slow and you stab it
        hit=(your_to_hit >= defender->evasion || stabAttempt)
        || ((defender->speed_increment <= 60) && (!one_chance_in(10 + you.skills[SK_STABBING])));       
    else 
       hit=(your_to_hit >= defender->evasion || one_chance_in(30))
        || ((defender->speed_increment <= 60
    ...
    if (hit)        
    {
       ...
       */
        ...
        damage*=calc_stat_to_dam_base(-1,78)/78;
...
#define STAFF_COST 2
    ...
    if (ur_armed && hit && item_is_staff( you.inv[weapon] ))
    {
        specdam = 0;
...
void monster_die(struct monsters *monster, char killer, int i)
{
    ...
    //alex:12.11.01
    int exp=exper_value(monster);
    ActEstadisticas(monster,monster_killed,exp);
                              ...
                              int damage)
{
    ...
    if (damage >= HIT_WEAK)
    {
        ...
        {
            if (item_is_staff( you.inv[weapnum] ))
...
{   
    ...
    //alex: 11.07.01
    //Before, this func. only works with the wielded weapon     
    int item_class,item_type;
    
    if (weapon == -1) weapon = you.equip[ EQ_WEAPON ];
    
    item_class = you.inv[weapon].base_type;
    item_type =  you.inv[weapon].sub_type;
    ...
    int ret = weapon_str_weight( item_class, item_type );

    const bool shield     = (you.equip[EQ_SHIELD] != -1);
    const int  hands_reqd = hands_reqd_for_weapon( item_class,item_type );
...
static int player_weapon_dex_weight( int weapon = -1)
{
    return (10 - player_weapon_str_weight(weapon));
}

int calc_stat_to_hit_base( int weapon = -1)
{
//#ifdef USE_NEW_COMBAT_STATS
if (xtraOps.USE_NEW_COMBAT_STATS) //{alex:12.11.01}
 { 
    ...
    //{alex:18.07.01} Now es 15+(x/2). Before was "x"
    return 15+(((you.dex + towards_str_avg * player_weapon_str_weight(weapon) / 10))/2);
 }    
//#else //{alex:12.11.01}
else
    return 15+(you.dex/2);
...
int calc_stat_to_dam_base( int weapon = -1,int dammod=78 ,int mode=RM_RANDOM)
{
	
int dam_stat_val;
signed int towards_dex_avg;

//#ifdef USE_NEW_COMBAT_STATS //{alex:12.11.01}
if (xtraOps.USE_NEW_COMBAT_STATS)
 {
    towards_dex_avg = (you.dex - you.strength) / 2;
    dam_stat_val= (you.strength + towards_dex_avg * player_weapon_dex_weight(weapon) / 10);
 }   
//#else
else
    dam_stat_val=dammod*(you.strength);
//#endif

if (dam_stat_val > 11)
   dammod += (rnd_modenize(dam_stat_val - 11) * 2,mode);
else if (dam_stat_val < 9)
   dammod -= (rnd_modenize(9 - dam_stat_val) * 3,mode);
