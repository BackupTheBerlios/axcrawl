 ...
 *
 *      <9>      21.03.02      ALX     .- Take into account resting
 *                                     .- option to show damage done to monsters by you
 *                                     .- detailed description of (possible) death
 *                                      
 *
...
static void handle_behaviour(struct monsters *mon)
{
    ...
    //if a monster not friendly is near you, stop resting {alex:15.11.01}
    if (isMobile && !isFriendly && proxPlayer) stop_resting("A monster is near you. You stop resting.");
...
//    losight(show, grd, you.x_pos, you.y_pos);
    ...
    case MONS_LAVA_SNAKE:
        ...
        beem.thrower=KILLED_BY_GLOB_LAVA; //alex:20.03.02
    ...
    case MONS_ELECTRICAL_EEL:
        ...
        beem.beam_source = monster_index(monster);
        //beem.thrower = KILL_MON;
        beem.thrower=KILLED_BY_ELECTRICITY; //alex:20.03.02
        beem.range = 4;
        beem.rangeMax = 13;
        beem.isBeam = true;
    ...
    case MONS_MANTICORE:
        ...
        //beem.thrower = KILL_MON;
        beem.thrower=KILLED_BY_VOLLEY_OF_SPIKES; //alex:20.03.02
        beem.isBeam = false;
...
static bool plant_spit(struct monsters *monster, struct bolt &pbolt)
{
    ...
    pbolt.thrower=KILLED_BY_SPIT_ACID; //alex:20.03.02
...
bool hurt_monster(struct monsters * victim, int damage_dealt)
{
    ...
    if (damage_dealt > 0)
    {
        ...
        //Show damage done {alex:19.09.01}
        //Without you_turn, it will be displayed damage that
        //monsters does one to other
        if (xtraOps.ax_show_dam && your_turn==1)
          {
              strcpy(info, "Damage for ");
              itoa(damage_dealt, st_prn, 10);   
              strcat(info, st_prn);
              strcat(info, ".");
              mpr(info);  
          }	
...
}                               // end hurt_monster()                              // end hurt_monster()
